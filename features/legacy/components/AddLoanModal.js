...
import { addLoan, updateLoan } from '@/app/actions/loanActions';
...
export default function AddLoanModal({ isOpen, onClose, initialData = null, existingNames = [] }) {
    ...
    const totalExtraCost = useMemo(() => {
        return Math.max(0, totalPayable - principalBase);
    }, [totalPayable, principalBase]);

    const interestAmount = useMemo(() => {
        return Math.max(0, totalExtraCost - totalUpfront);
    }, [totalExtraCost, totalUpfront]);

    const chargesExceedError = useMemo(() => {
        if (totalUpfront - totalExtraCost > 0.01) {
            return `Processing Fee + GST + Other Charges cannot exceed extra payable amount (${formatCurrency(totalExtraCost)}).`;
        }
        return '';
    }, [totalUpfront, totalExtraCost]);

    const payableMismatch = useMemo(() => {
        if (principalBase <= 0) return '';
        if (totalPayable + 0.01 < principalBase) {
            return 'Total payable cannot be less than amount credited.';
        }
        return '';
    }, [principalBase, totalPayable]);
    ...
    useEffect(() => {
        ...
        if (principalBase <= 0) {
            setInterestRateError('Amount credited must be positive to calculate rate.');
            setRawInterestRate(0);
            setFormData(prev => (prev.interestRate === '0.00' ? prev : { ...prev, interestRate: '0.00' }));
            return;
        }

        const simpleRate = (interestAmount / principalBase) * 100;
        setRawInterestRate(simpleRate);
        setInterestRateError('');
        const nextRate = simpleRate.toFixed(2);
        setFormData(prev => (prev.interestRate === nextRate ? prev : { ...prev, interestRate: nextRate }));
    }, [isOpen, principalBase, interestAmount]);

    useEffect(() => {
        setReconcileError(chargesExceedError || payableMismatch);
    }, [chargesExceedError, payableMismatch]);
                ...
                <form className={styles.form} onSubmit={handleSubmit} noValidate>
                        ...
                        <>
                            ...
                            <div className={styles.scheduleWrapper}>
                                <div className={styles.scheduleHeader}>
                                    ...
                                    <span>Date</span>
                                    <span>Amount</span>
                                    <span className={styles.paidHeader}>Paid</span>
                                </div>
                        ...
                        <div className={styles.insightSection}>
                            <div className={styles.insightGrid}>
                                <div className={styles.insightCard}>
                                    ...
                                    <span className={styles.interestCaption}>
                                        Interest: {formatCurrency(interestAmount)}
                                    </span>
                                    {tinyRateHint && (
                                        <span className={styles.tinyHint}>
                                            {tinyRateHint}
                                        </span>
                                    )}
                                    {interestRateError && <span className={`${styles.errorMessage} ${styles.centeredError}`}>{interestRateError}</span>}
                                </div>
                            ...
                            <div className={styles.row}>
                                <div className={styles.formGroup}>
                                    ...
                                    <input
                                        ...
                                        placeholder="0.00"
                                    />
                                    <label className={`${styles.checkboxRow} ${styles.gstLabel}`}>
                                        <input
                                            name="gstIncluded"
                                            type="checkbox"
                                    ...
                                    <span className={styles.gstValue}>
                            ...
                            <div
                                ...
                                style={{ borderColor: advice.color, color: advice.color, background: `${advice.color}12` }}
                            >
                    ...
                    <div className={styles.actions}>
                        ...
                        {step < 3 ? (
                            ...
                            <button
                                type="submit"
                                disabled={isSubmitting || !!reconcileError || !!creditedAmountError || !!scheduleError || !!paidToggleError}
                                className={styles.submitBtn}
                            >
                    ...
                    {step < 3 && showNextError && nextDisabledReason && (
                        <span className={`${styles.errorMessage} ${styles.nextError}`}>
                            {nextDisabledReason}
                        </span>
                    )}
                </form>
